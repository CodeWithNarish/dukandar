// lib/presentation/screens/weight_to_price/weight_to_price_screen.dart
...
class WeightToPriceScreen extends ConsumerStatefulWidget {
  const WeightToPriceScreen({super.key});

  @override
  ConsumerState<WeightToPriceScreen> createState() => _WeightToPriceScreenState();
}

class _WeightToPriceScreenState extends ConsumerState<WeightToPriceScreen> {
  ...
  // Local state for the customer's input unit
  String _customerQtyUnit = 'KG';
  double _rawCustomerQtyInput = 0.0;
  ...
  // Trigger calculation when input values or units change
  void _recalculate(WeightToPriceState state, double rawQty, String rawUnit) {
    final notifier = ref.read(weightToPriceCalculatorProvider.notifier);
    
    // If the input unit is different, convert it to the product's base unit
    double convertedQty = rawQty;
    if (CalculationEngine.areUnitsCompatible(rawUnit, state.unit)) {
      convertedQty = CalculationEngine.convertUnit(
        value: rawQty,
        fromUnit: rawUnit,
        toUnit: state.unit,
      );
    ...
    notifier.updateCustomerQuantity(convertedQty);
  }

  void _saveToHistoryOnPop(WeightToPriceState state) {
    final settingsAsync = ref.read(settingsNotifierProvider);
    final autoSave = settingsAsync.value?.autoSaveHistory ?? true;
    
    if (autoSave && state.price > 0 && state.customerQuantity > 0 && state.result > 0) {
      final entry = HistoryModel()
        ..productName = state.productName.isEmpty ? 'Weight → Price Calc' : state.productName
        ..calculationType = 'weight_to_price'
        ..inputPrice = state.price
        ..inputQuantity = state.customerQuantity
        ..unit = state.unit
        ..inputAmount = state.result
  ...
  Widget build(BuildContext context) {
    ...
    final state = ref.watch(weightToPriceCalculatorProvider);
    final notifier = ref.read(weightToPriceCalculatorProvider.notifier);

    // Sync controllers with state if state changes from favorite prefill
    ref.listen(weightToPriceCalculatorProvider, (previous, next) {
      ...
      if (previous?.unit != next.unit) {
        setState(() {
          _customerQtyUnit = next.unit;
        });
      }
    });

    final double pricePerUnit = state.price / state.baseQuantity;
    final String formulaString = _customerQtyUnit == state.unit
        ? '₹${state.price} ÷ ${state.baseQuantity}${state.unit} = ₹${pricePerUnit.toStringAsFixed(2)} per ${state.unit}\n'
            '₹${pricePerUnit.toStringAsFixed(2)} × $_rawCustomerQtyInput${state.unit} = ${NumberFormatter.formatCurrency(state.result)}'
        : '₹${state.price} ÷ ${state.baseQuantity}${state.unit} = ₹${pricePerUnit.toStringAsFixed(2)} per ${state.unit}\n'
            'Customer weight converted: $_rawCustomerQtyInput $_customerQtyUnit = ${state.customerQuantity.toStringAsFixed(3)} ${state.unit}\n'
            '₹${pricePerUnit.toStringAsFixed(2)} × ${state.customerQuantity.toStringAsFixed(3)} ${state.unit} = ${NumberFormatter.formatCurrency(state.result)}';
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(LocalizationHelper.translate('weight_to_price', lang)),
          actions: [
            IconButton(
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Product Name Input + Suggestions
                      Focus(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return ListTile(
                                      ...
                                      onTap: () {
                                        ...
                                        setState(() {
                                          _customerQtyUnit = product.unit;
                                          _showSuggestions = false;
                                        });
                                        _recalculate(state, _rawCustomerQtyInput, _customerQtyUnit);
                      ...
                      // Customer weight / quantity inputs
                      Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Expanded(
                            ...
                            child: PremiumTextField(
                              ...
                              label: LocalizationHelper.translate('customer_weight', lang),
                              autofocus: true,
                              keyboardType: const TextInputType.numberWithOptions(decimal: true),
                              onChanged: (val) {
                                final parsed = double.tryParse(val) ?? 0.0;
                                setState(() {
                                  _rawCustomerQtyInput = parsed;
                                });
                                _recalculate(state, parsed, _customerQtyUnit);
                              },
                            ),
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            flex: 2,
                            child: UnitSelector(
                              label: lang == 'hi' ? 'इकाई' : 'Input Unit',
                              selected: _customerQtyUnit,
                              onChanged: (val) {
                                setState(() {
                                  _customerQtyUnit = val;
                                });
                                _recalculate(state, _rawCustomerQtyInput, val);
                              },
                            ),
                          ),
                        ],
                      ...
                      // Result Card Display
                      if (state.price > 0 && state.customerQuantity > 0)
                        ResultCard(
                          value: state.result,
                          unit: '₹',
                      ...
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: () {
                            ...
                            final resultStr = NumberFormatter.formatCurrency(state.result);
  ...
  void _saveAsProduct(BuildContext context, WeightToPriceState state, String lang) {
